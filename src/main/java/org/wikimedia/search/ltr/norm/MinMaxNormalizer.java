...
public class MinMaxNormalizer extends Normalizer {
    ...
    public MinMaxNormalizer(Settings params) {
        min = params.getAsFloat("min", Float.NEGATIVE_INFINITY);
        max = params.getAsFloat("max", Float.POSITIVE_INFINITY);
        delta = max - min;
    }
    ...
    protected void validate() throws NormalizerException {
        ...
        if (delta == 0f)
            throw new NormalizerException(
                "MinMax Normalizer delta must not be zero " + "| min = " + min + ",max = " + max + ",delta = " + delta);
    }
